...
struct sMailInfo //邮件信息  
{  
	char   m_pcUserName[128];//用户登录邮箱的名称  
	char   m_pcUserPassWord[128];//用户登录邮箱的密码  
	char   m_pcSenderName[128];//用户发送时显示的名称  
	char   m_pcSender[128];//发送者的邮箱地址  
	char   m_pcReceiver[128];//接收者的邮箱地址  
	char   m_pcTitle[128];//邮箱标题  
	char   m_pcBody[1024*10];//邮件文本正文  
	char   m_pcIPAddr[128];//服务器的IP  
	char   m_pcIPName[128];//服务器的名称（IP与名称二选一，优先取名称）  
	sMailInfo(){memset(this,0,sizeof(sMailInfo));}  
}; 
class CSendMail  
{  
...
protected:  
    CList<char*,char*> m_FilePathList;//记录附件路径  
  
    char  m_SendBuff[4096];//发送缓冲区  
    char  m_cReceiveBuff[1024];  
    char* m_pFileBuffer;//指向附件内容  
