...
std::string AppOctStrmContent::encode(std::string in_str)
{
	std::string out_str;
...
std::string MailWrapper::getHeader()
{
	...
	msg += "Subject: ";
	msg += _mail_info->getSubject() + "\r\n";

	msg += "X-Mailer: ";
	msg += _mailer_name + "\r\n";

	msg += "X-Priority: ";
	msg += _mail_info->getPriority() + "\r\n";
...
std::string MailSender::Encode(std::string in_str)
{
	std::string out_str;
	unsigned char c1, c2, c3;
	int i = 0;
...
void MailSender::login()
{
	...
	msg = Encode(_user_name) + "\r\n";
    sendRequest(send_username, msg);
    rcvResponse("334");

	msg = Encode(_user_pwd) + "\r\n";
