				...
				case SEND_POSTED:
					{
						...
						pIOCPModel->_DoSendComplete(pSocketContext,pIoContext);
						pIOCPModel->_ShowMessage("成功发送了【%d】数据",dwBytesTransfered);
						break;
					}
				default:
					// 不应该执行到这里
...
bool CIOCPModel::_DoAccpet( PER_SOCKET_CONTEXT* pSocketContext, PER_IO_CONTEXT* pIoContext )
{
	...
	if(strcmp(pIoContext->m_wsaBuf.buf,"RecvThread") != 0 )
	{
		///////////////////////////////////////////////////////////////////////////////////////////////////
	...
	else
	{
		...
		if(m_pRecvContext == NULL)
		{
			m_pRecvContext = pNewSocketContext;
		//	m_recvSocket = pIoContext->m_sockAccept;
		}
		::LeaveCriticalSection(&m_csLock);
	}
...
/////////////////////////////////////////////////////////////////
// 投递发送数据请求
bool CIOCPModel::_PostSend(std::string szData )
{
	::EnterCriticalSection(&m_csLock);
	if(m_pRecvContext == NULL)
	{
		this->_ShowMessage("接收客户端未接入，发送失败!");
		return false;
	}
	...
	FD_SET(m_pRecvContext->m_Socket,&fds); //添加描述符 
	int nwrite = select(0,NULL,&fds,NULL,&timeout);
	if(nwrite == -1)
	{
//		RELEASE(m_pRecvContext);
		closesocket(m_pRecvContext->m_Socket);
		_ShowMessage("无效socket，取消发送:%s",szData.c_str());
		return false;
	}

	//初始化 
	DWORD dwBytes = 0;		
	PER_IO_CONTEXT *sendContext = m_pRecvContext->GetNewIoContext();// new PER_IO_CONTEXT;

	sendContext->m_sockAccept = m_pRecvContext->m_Socket;
	sendContext->m_OpType = SEND_POSTED;
	ZeroMemory(&sendContext->m_Overlapped,sizeof(sendContext->m_Overlapped));
	sendContext->m_wsaBuf.len = szData.length();
	lstrcpyn(sendContext->m_szBuffer,szData.c_str(),sizeof(sendContext->m_szBuffer));

	int nBytesSend = WSASend( sendContext->m_sockAccept,&sendContext->m_wsaBuf , 1, &dwBytes, 0, &sendContext->m_Overlapped, NULL );

	// 如果返回值错误，并且错误的代码并非是Pending的话，那就说明这个重叠请求失败了
	if ((SOCKET_ERROR == nBytesSend) && (WSA_IO_PENDING != WSAGetLastError()))
	{
		this->_ShowMessage("投递WSASend失败！LastError:%d",WSAGetLastError());
		return false;
	}
	...
	lstrcpyn(szSend,szData.c_str(),sizeof(szSend));
	int nBytesSent = send(pRecvContext->m_Socket, szData.c_str(), szData.length(), 0);
	if (SOCKET_ERROR == nBytesSent) 
	{
		TRACE("错误：发送Recv线程注册信息失败，错误代码：%ld\n", WSAGetLastError());
		return 1; 
	}
	*/
...
bool CIOCPModel::_DoRecv( PER_SOCKET_CONTEXT* pSocketContext, PER_IO_CONTEXT* pIoContext )
{
	...
	char buff[1024] = {0};
	sprintf(buff,"【 %s:%d 】：%s",inet_ntoa(ClientAddr->sin_addr), ntohs(ClientAddr->sin_port),pIoContext->m_wsaBuf.buf );
	this->_ShowMessage( _T(buff));
...
bool CIOCPModel::_DoSendComplete(PER_SOCKET_CONTEXT* pSocketContext, PER_IO_CONTEXT* pIoContext )
{
	struct fd_set fds; 
	struct timeval timeout={3,0}; //select等待3秒，3秒轮询，要非阻塞就置0 
	FD_ZERO(&fds); //每次循环都要清空集合，否则不能检测描述符变化 
	FD_SET(pIoContext->m_sockAccept,&fds); //添加描述符 
	int nwrite = select(0,NULL,&fds,NULL,&timeout);
	if(nwrite == -1)
	{
		RELEASE(pSocketContext);
		pSocketContext=NULL;
		_ShowMessage("发送完成后，socket失效了！");
		return false;
	}	
