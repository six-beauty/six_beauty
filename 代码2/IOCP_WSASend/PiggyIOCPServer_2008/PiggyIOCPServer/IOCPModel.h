...
注意！！！！！！！！！！！！！！！！！！！！！！！！
由于下面结构体里面的 构造函数  使用 m_wsaBuf.buf = m_szBuffer 
必须使用 new来为结构体
*/
...
typedef struct _PER_IO_CONTEXT
{
	...
	WSABUF         m_wsaBuf;                                   // WSA类型的缓冲区，用于给重叠操作传参数的，这个buf留给 Iocp用的，存 m_szBuffer的地址，读、写应该对m_szBuffer进行
...
protected:
	...
	// 投递发送数据请求
	bool _PostSend(std::string szData );
	...
	bool _DoSendComplete(PER_SOCKET_CONTEXT* pSocketContext, PER_IO_CONTEXT* pIoContext );
...
private:
	inline void Lock(CRITICAL_SECTION stLock)	{ ::EnterCriticalSection(&stLock);}

	inline void UnLock(CRITICAL_SECTION stLock)	{ ::EnterCriticalSection(&stLock);}
...
private:
	...
	PER_SOCKET_CONTEXT*			 m_pRecvContext;                // 客户端的接收线程Socket的Context信息
