...
// 产生Socket连接的线程
typedef struct _tagThreadParams_Client
{
	CClient* pClient;                               // 类指针，用于调用类中的函数

} THREADPARAMS_Client,*PTHREADPARAM_CLIENT; 
...
private:
	...
	// 用于接收信息的线程
	static DWORD WINAPI _RecvThread(LPVOID lpParam);
...
private:
	...
	HANDLE    m_hRecvThread;	                            // 接受服务的信息的线程句柄
