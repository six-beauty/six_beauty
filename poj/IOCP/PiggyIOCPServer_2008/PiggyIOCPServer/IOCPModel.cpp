...
bool CIOCPModel::_DoAccpet( PER_SOCKET_CONTEXT* pSocketContext, PER_IO_CONTEXT* pIoContext )
{
	...
	{
		m_arrayRecvContext = pNewSocketContext;
	}
...
bool CIOCPModel::_PostSend(std::string szData )
{
	...
	*/
	...
	int nBytesSent = send(m_arrayRecvContext->m_Socket, szData.c_str(), szData.length(), 0);
...
bool CIOCPModel::_DoSendComplete(PER_SOCKET_CONTEXT* pSocketContext, PER_IO_CONTEXT* pIoContext )
{
	...
	ZeroMemory(szRecv,MAX_BUFFER_LEN);
...
void CIOCPModel::_AddToContextList( PER_SOCKET_CONTEXT *pHandleData )
{
	...
	m_arrayRecvContext.Add(pHandleData);	
